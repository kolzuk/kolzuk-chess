...
object MoveGenerator {
  ...
  def perft(depth: Int, board: Board, isFirstCall: Boolean = true)(implicit moveGenerator: MoveGenerator): Long = {
    def recursivePerft(currentDepth: Int, currentBoard: Board, isFirstCall: Boolean): Long = {
      if (currentDepth == 0) {
        1L
      } else {
        ...
        if (currentDepth == 1) {
          val allMovesCount = moves.map(m => {
            val countOfMoves = recursivePerft(currentDepth - 1, currentBoard.makeMove(m), isFirstCall = false)
            if (isFirstCall)
    ...
    recursivePerft(depth, board, isFirstCall)
  }
}